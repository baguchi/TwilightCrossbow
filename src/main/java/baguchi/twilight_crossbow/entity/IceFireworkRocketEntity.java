...
public class IceFireworkRocketEntity extends Projectile implements ItemSupplier {
    private static final EntityDataAccessor<ItemStack> DATA_ID_FIREWORKS_ITEM = SynchedEntityData.defineId(
            IceFireworkRocketEntity.class, EntityDataSerializers.ITEM_STACK
    );
    private static final EntityDataAccessor<OptionalInt> DATA_ATTACHED_TO_TARGET = SynchedEntityData.defineId(
            IceFireworkRocketEntity.class, EntityDataSerializers.OPTIONAL_UNSIGNED_INT
    );
    private static final EntityDataAccessor<Boolean> DATA_SHOT_AT_ANGLE = SynchedEntityData.defineId(IceFireworkRocketEntity.class, EntityDataSerializers.BOOLEAN);
    private int life;
        ...
        if (compound.contains("FireworksItem", 10)) {
            this.entityData
                    .set(
                            ...
                            ItemStack.parse(this.registryAccess(), compound.getCompound("FireworksItem")).orElseGet(IceFireworkRocketEntity::getDefaultItem)
                    );
        } else {
